...
class Tokenizer:
    ...
    def __init__(self,
        ...
        if granularity == "word":
            ...
            self.ltp = LTP("/new_disk3/chen_zhang/zc/MuCGEC/scorers/ChERRANT/LTP/small")
    ...
    #     else:
    #         # seg, hidden = self.ltp.segment(input_strings)
    #         output = ltp.pipeline(input_strings, tasks=["cws", "pos", "ner", "srl", "dep", "sdp"])
    ...
    def split_word(self, input_strings: List[str]) -> List:
        """
        新版分词函数（兼容预分词模式）
        :param input_strings: 需要处理的字符串列表
        :return: 结构化的分词结果
        """
        # 处理预分词模式
        if self.segmented:
            # 将预分词的字符串转换为二维列表
            pre_segmented = [s.split() for s in input_strings]
            # 只进行词性标注和后续处理
            pos = self.ltp.pos(pre_segmented)
            return self._process_results(pre_segmented, pos)
        
        # 正常处理流程

        output = self.ltp.pipeline(
            input_strings,
            tasks=["cws", "pos"],  # 明确指定任务顺序
            return_dict=False  # 确保返回元组格式
        )
        # 元组结构解析：output = (cws_results, pos_results, ...)
        return self._process_results(output[0], output[1])  # 用索引替代属性

    def _process_results(self, cws_results, pos_results):
        final_results = []
        for words, pos_tags in zip(cws_results, pos_results):
            pinyin = [lazy_pinyin(word) for word in words]
            final_results.append(list(zip(words, pos_tags, pinyin)))
